...
**Owner: P1.** React + Vite (`horizon-ui/`), offline, no external calls at inference. Forecasts ship as static JSON under `public/mock/`; `VITE_API_BASE` optionally points at the live FastAPI backend (`horizon-api/`).

**Status: built, v2.** All five panels plus a network scene. This document is the design intent; the panel specs below still hold. Added since v1:

- **Two themes**, toggle in the header: Azure light (default) and a tactical dark (near-black, gold primary, green standout). Persisted per viewer.
- **Chart hover readouts** on the cone, surprise track, and counterfactual curves.
- **Panel explain** — an `(i)` popover on every panel header (what it shows + what to say to a judge). Full prep sheet: `docs/dashboard-guide.md`.
- **Focus mode** — a `⤢` control on each panel opens it fullscreen alongside its connected panels; Esc closes.
- **Real network scene** — macro host-communication graph from `GET /api/network` (real nodes and edges), node hover tooltips, a shared **playhead** scrub/play control that also marks the forecast cone, the attack front spreading along the graph, and the counterfactual choice visibly cutting the attack edges. Click a node for the micro view: its real flows from `GET /api/flows`. (The earlier r3f 3D scene was replaced; the real topology reads better flat.)
- **Intro screens** — 4-slide project intro on first load, Replay in the header.
- **8-stage MITRE** kill chain, aligned with `horizon_api/mitre.py`.

Output shapes frozen in `api-contract.md` (§3.7 network, §3.8 flows added).
...
- Forecasts for the demo hosts are precomputed and shipped as static JSON (`public/mock/`, generated by `gen_mock.py`; replaced by real output from the notebook + backend). Live inference on stage is an unnecessary risk; the static bundle is the default transport.
- Charts are canvas, `ResizeObserver`-driven. Nothing should take three seconds; the JSON loads are small.
- Test with the sidebar collapsed and on a projector at 1280×720. Thin lines and low-contrast greys vanish on a projector.
- Handle missing fields gracefully — if a counterfactual branch is absent, the panel says so rather than crashing.
- Keep a known-good frozen demo build (`npm run build` output) separate from the one being edited.
- For a weak-GPU demo machine, `localStorage.horizon.scene2d = '1'` forces the 2D fallback scene.
