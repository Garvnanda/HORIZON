...
| **Primary dataset** | **CIC-IDS2017 GeneratedLabelledFlows** (Kaggle: `pshikk/cicids2017-untampered`, backup `rayenbal/cicids2017`) — 8 CICFlowMeter CSVs across five weekday capture sessions, with Flow ID, both IPs, both ports, timestamp, and label | `cic-collection.parquet` (four harmonised CIC campaigns) was the v4 pick but drops IPs and timestamp. Most Kaggle "CIC-IDS2017" uploads (`chethuhn/...`, `kk0105/...`) are the MachineLearningCVE variant, also stripped — they fail Gate 0. The "untampered" upload keeps the raw CICFlowMeter columns. Fits in RAM with chunked reads. |
